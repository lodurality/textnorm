...
model_name = 'fourth_run_downsample_common'
...
train_pairs = get_pairs(data_learn, downsample_common = False)
...
plot_losses = trainIters_weighted(encoder1, attn_decoder1, train_pairs, test_pairs, 2*num_iterations, print_every=callback_num,
                         plot_every=callback_num, evaluate_each=20000, learning_rate = 0.00001,
                        min_class_size = 800, add_weighted = False)

print('LR 0.000001')
plot_losses = trainIters_weighted(encoder1, attn_decoder1, train_pairs, test_pairs, 2*num_iterations, print_every=callback_num,
                         plot_every=callback_num, evaluate_each=20000, learning_rate = 0.000001,
                        min_class_size = 800, add_weighted = False)



torch.save(encoder1.state_dict() , 'models/encoder_{}_{}_iters.states'.format(model_name, num_iterations))
torch.save(attn_decoder1.state_dict(), 'models/decoder_{}_{}_iters.states'.format(model_name, num_iterations))
